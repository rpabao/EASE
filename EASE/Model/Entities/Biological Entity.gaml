...
species BiologicalEntity control:fsm{
	...
	//Time between first infection and reinfection
	float natural_immunity_period;
	...
	bool is_symptomatic;
	// Boolean to determine if the agent has natural immunity already
	bool has_natural_immunity <- false;
	
	//Boolean to determine if the agent is vaccinated 1st dose
	bool is_vaccinated_1stdose <- false;
	//Boolean to determine if the agent is vaccinated 2nd dose
	bool is_vaccinated_2nddose <- false;
	//Boolean to determine if the agent is vaccinated booster
	bool is_vaccinated_booster <- false;
	...
	action set_as_vaccinated_1stdose{
		is_vaccinated_1stdose <- true;
	}
	
	action set_as_vaccinated_2nddose{
		is_vaccinated_2nddose <- true;
	}
	
	action set_as_vaccinated_booster{
		is_vaccinated_booster <- true;
	}
	...
	//Return the fact of the Individual showing any symptoms (i.e. asymptomatic, presymptomatic)
	bool define_is_symptomatic {
		return [symptomatic] contains state;
	}
	...
	*/
	...
	state latent {
		...
		transition to: symptomatic when: (tick>=latent_period) and (self.is_symptomatic);
	...
	*/
	...
	state removed{
		enter{
			tick <- 0.0;
			do set_status;
			natural_immunity_period <- world.get_reinfection_interval(age);
		}
		tick <- tick+1;
		transition to:susceptible when: (tick>=natural_immunity_period){
			is_hospitalised <- false;
			clinical_status <- no_need_hospitalisation;
			has_natural_immunity <- true;
		}
	}
