...
species Authority {
	...
	int daycounter <- 1; //for the counting of days elapsed needed for batch implementation
	int breaktracker <- 1; //for the counting or tracking of number of break days elapsed
	bool isBreak <- false; //whether it is a break or not
	
	reflex setBatchToEnter{
		//if the current cycle represents a new day and if it is a working day
		//Note: this assumes that each cycle represents an hour
		if (cycle mod 24 = 0 and !(non_working_days one_matches (each = current_date.day_of_week))) {
			if (isBreak = true) { //during batch break
				breaktracker <- breaktracker + 1; //increment number of break days			
				//check if break is over
				if (breaktracker > daysinterval) {
					isBreak <- false; //break is over
					breaktracker <- 1; //reset break days count
					batchtoenter <- 1; //reset batch to enter
				}
			}
			else {	//on regular days			
				//if the current batch has completed their number of days
				if (daycounter > daysperbatch){
					batchtoenter <- batchtoenter + 1; //move to the next batch
					//if all batches have finished its scheduled days
					if (batchtoenter > batchcount){
						//if batch break is implemented
						if (daysinterval > 0) {
							isBreak <- true; // take a break
							batchtoenter <- 0; //no one is allowed to enter
						}
						//if no break implemented
						else {
							batchtoenter <- 1;
						}
					}
					daycounter <- 1; //reset day counter
				}
				daycounter <- daycounter + 1; //increment day counter per batch
			}
		}
	}
