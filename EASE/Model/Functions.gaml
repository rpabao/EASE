...
global
{
	...
	float get_prob_infection_outside(int age)
	{
		return (float(map_epidemiological_parameters[age][epidemiological_Rt][1]) / get_infectious_period_symptomatic(age));
	}
	...
	*/
	...
	float get_reinfection_interval(int age)
	{
		if(map_epidemiological_parameters[age][epidemiological_reinfection_interval][0]=epidemiological_fixed)
		{
			return float(map_epidemiological_parameters[age][epidemiological_reinfection_interval][1])*nb_step_for_one_day;
		}
		else
		{
			return get_rnd_from_distribution(map_epidemiological_parameters[age][epidemiological_reinfection_interval][0],float(map_epidemiological_parameters[age][epidemiological_reinfection_interval][1]),float(map_epidemiological_parameters[age][epidemiological_reinfection_interval][2]))*nb_step_for_one_day;
		}
	}
